...
public class DashboardController {
    ...
    private void loadExpensesAndIncomes(LocalDate currentMonth) {
        LinkedHashMap<String, Pair<Number, Number>> totalsByMonth = new LinkedHashMap<>();
        ...
        for (Expense expense : lastExpenses) {
            ...
            Pair<Number, Number> totals = totalsByMonth.getOrDefault(monthKey, new Pair<>(0f, 0f));
            float updatedExpenses = totals.getKey().floatValue() + expense.getTotal();
            totalsByMonth.put(monthKey, new Pair<>(updatedExpenses, totals.getValue()));
        }
        ...
        for (Income income : lastIncomes) {
            ...
            Pair<Number, Number> totals = totalsByMonth.getOrDefault(monthKey, new Pair<>(0f, 0f));
            float updatedIncomes = totals.getValue().floatValue() + income.getTotal();
            totalsByMonth.put(monthKey, new Pair<>(totals.getKey(), updatedIncomes));
        }
        ...
        for (Map.Entry<String, Pair<Number, Number>> entry : totalsByMonth.entrySet()) {
            String month = entry.getKey();
            Pair<Number, Number> totals = entry.getValue();
